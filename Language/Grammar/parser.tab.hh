...
# if 201103L <= YY_CPLUSPLUS
  ...
  private:
    ...
    {
      // COMPARING
      char dummy1[sizeof (INode::ComparingOp)];
      ...
      // condition_and
      // condition_brace
      char dummy2[sizeof (INode::INode_t*)];
...
#endif
#if 201103L <= YY_CPLUSPLUS
      basic_symbol (typename Base::kind_type t, INode::ComparingOp&& v)
        : Base (t)
        , value (std::move (v))
      {}
#else
      basic_symbol (typename Base::kind_type t, const INode::ComparingOp& v)
        : Base (t)
        , value (v)
      {}
#endif
...
switch (yytype)
    {
      case 5: // COMPARING
        value.template destroy< INode::ComparingOp > ();
        break;
    ...
    struct symbol_type : basic_symbol<by_type>
    {
      ...
      {
        YY_ASSERT (tok == 0 || tok == token::WHILE || tok == token::IF || tok == token::SCOLON || tok == token::LB || tok == token::RB || tok == token::LB_FIG || tok == token::RB_FIG || tok == token::OUTPUT || tok == token::ASSIGN || tok == token::INPUT || tok == token::OR || tok == token::AND || tok == token::NOT || tok == token::PLUS || tok == token::MINUS || tok == token::MULT || tok == token::DIV || tok == token::MOD);
      }
#else
      symbol_type (int tok)
        : super_type(token_type (tok))
...
#endif
#if 201103L <= YY_CPLUSPLUS
      symbol_type (int tok, INode::ComparingOp v)
        : super_type(token_type (tok), std::move (v))
      {
        YY_ASSERT (tok == token::COMPARING);
      }
#else
      symbol_type (int tok, const INode::ComparingOp& v)
    ...
    // Implementation of make_symbol for each symbol type.
      ...
      static
      symbol_type
      make_COMPARING (INode::ComparingOp v)
      {
        return symbol_type (token::COMPARING, std::move (v));
      }
#else
      static
      symbol_type
      make_COMPARING (const INode::ComparingOp& v)
      {
        return symbol_type (token::COMPARING, v);
      }
#endif
#if 201103L <= YY_CPLUSPLUS
      static
      ...
      make_LB_FIG ()
      {
        return symbol_type (token::LB_FIG);
      }
#else
      static
      symbol_type
      ...
      make_RB_FIG ()
      {
        return symbol_type (token::RB_FIG);
      }
#else
      static
      symbol_type
