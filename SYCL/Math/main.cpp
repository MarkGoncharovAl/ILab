...
static void PrintResult (Msycl::Vector const& vec);
...
int main ()
{
    try
    {
        ...
        Msycl::Matrix<float> cl_matrix (answer.first); //diagonal fast matrix
        Msycl::Vector cl_vector (std::move (answer.second)); //convenient wrapper
        auto&& final_result = Msycl::Dirichle {}.Solve (cl_matrix , cl_vector);
...
void PrintResult (const Msycl::Vector& vec)
{
    std::cout << "GPU result:\n";
